...
from surrogate import DeepPermutation
from utils import get_ann_benchmark, nice_logspace, compute_recalls


def load_or_train_index(x, C, R, trained_index_path, train_time_path):
    if not Path(trained_index_path).exists() or args.force:
        # train common index
        logging.info('Training index ...')
        d = x.shape[1]
        train_time = time.time()
        index = DeepPermutation(d, use_centroids=C, rectify_negatives=R)
        index.train(x)
...
def main(args):
    ...
    C, R = args.use_centroids, args.rectify_negatives

    trained_index_path = exp.path_to('empty_trained_index.pickle')
    train_time_path = exp.path_to('train_time.txt')
    
    index, train_time = load_or_train_index(x, C, R, trained_index_path, train_time_path)

    metrics_path = exp.path_to('metrics.csv.gz')
    metrics = pd.read_csv(metrics_path) if Path(metrics_path).exists() else pd.DataFrame()

    ## SPQR
    logging.info('Building, Searching, and Evaluating DeepPermutation index ...')

    with logging_redirect_tqdm():
        progress_L = tqdm(list(nice_logspace(d)), desc='DeepPerm')
        for L in progress_L:
            progress_L.set_postfix({'prefix': L})

            logging.info(f'DeepPerm C={C} R={R} L={L}')
            ...
            if not args.force:
                if not metrics.empty and 'permutation_length' in metrics.columns:
                    sel = (metrics['permutation_length'] == L)
                    if sel.any():
                        search_time = metrics[sel].iloc[0]['search_time']
                        logging.info(f'SKIPPING DeepPerm C={C} R={R} L={L}')
                        continue

            if search_time > args.search_timeout:
                logging.info(f'Skipping permutation_length >= {L}, search() would take too long (> {args.search_timeout} s).')
                break
            ...
            new_metrics = pd.DataFrame([{
                'index_type': 'dp-pivots',
                'train_time': train_time,
                'build_time': build_time,
                'num_entries': index.db.nnz,
                'permutation_length': L,
            ...
            k = 100
            logging.info(f'DeepPerm C={C} R={R} L={L}: R@{k}={recalls[k]:.3f} search-time: {search_time:2.2f} s')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run DeepPerm Experiments')
