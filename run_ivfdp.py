...
from surrogate import IVFDeepPermutation
from utils import get_ann_benchmark, nice_logspace, compute_recalls


def load_or_train_index(x, C, R, N, trained_index_path, train_time_path):
    if not Path(trained_index_path).exists() or args.force:
        # train common index
        logging.info('Training index ...')
        d = x.shape[1]
        train_time = time.time()
        index = IVFDeepPermutation(d, n_coarse_centroids=C, rectify_negatives=R, l2_normalize=N)
        index.train(x)
...
def main(args):
    ...
    C, R, N = args.n_coarse_centroids, args.rectify_negatives, args.l2_normalize

    trained_index_path = exp.path_to('empty_trained_index.pickle')
    train_time_path = exp.path_to('train_time.txt')
    
    index, train_time = load_or_train_index(x, C, R, N, trained_index_path, train_time_path)
    ...
    with logging_redirect_tqdm():
        progress_L = tqdm(list(nice_logspace(d)), desc='IVFDP')
        for L in progress_L:
            progress_L.set_postfix({'prefix': L})

            logging.info(f'IVFDP C={C} R={R} N={N} L={L}')
            ...
            for nprobe in progress_P:
                ...
                if not args.force:
                    if not metrics.empty and 'nprobe' in metrics.columns:
                        sel = (metrics['permutation_length'] == L) & (metrics['nprobe'] == nprobe)
                        if sel.any():
                            search_time = metrics[sel].iloc[0]['search_time']
                            logging.info(f'SKIPPING IVFDP C={C} R={R} N={N} L={L} nprobe={nprobe}')
                            continue
                ...
                k = 100
                logging.info(f'IVFDP C={C} R={R} N={N} L={L} nprobe={nprobe}: R@{k}={recalls[k]:.3f} search-time: {search_time:2.2f} s')
