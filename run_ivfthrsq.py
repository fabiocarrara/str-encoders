...
from surrogate import IVFThresholdSQ
from utils import get_ann_benchmark, nice_logspace, compute_recalls


def load_or_train_index(d, x, C, M, Q, S, R, N, trained_index_path, train_time_path):
    if not Path(trained_index_path).exists() or args.force:
        # train common index
        logging.info('Training index ...')
        train_time = time.time()
        index = IVFThresholdSQ(d, n_coarse_centroids=C, n_subvectors=M, threshold_percentile=Q, sq_factor=S, rectify_negatives=R, l2_normalize=N)
        index.train(x)
...
def main(args):
    ...
    C, M, Q, S, R, N = args.n_coarse_centroids, args.n_subvectors, args.threshold_percentile, args.sq_factor, args.rectify_negatives, args.l2_normalize

    trained_index_path = exp.path_to('empty_trained_index.pickle')
    train_time_path = exp.path_to('train_time.txt')
    
    index, train_time = load_or_train_index(d, x, C, M, Q, S, R, N, trained_index_path, train_time_path)
    ...
    ## IVFTHRSQ
    logging.info('Searching, and Evaluating IVFTHRSQ index ...')

    with logging_redirect_tqdm():
        logging.info(f'IVFTHRSQ C={C} M={M} Q={Q} S={S} R={R} N={N}')        
        ...
        for nprobe in progress_P:
            ...
            k = 100
            logging.info(f'IVFTHRSQ C={C} M={M} Q={Q} S={S} R={R} N={N} nprobe={nprobe}: R@{k}={recalls[k]:.3f}')
